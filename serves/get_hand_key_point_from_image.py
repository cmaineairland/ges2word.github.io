...
LastEditTime: 2024-05-27 17:04:32
FilePath: \gesrec\serves\get_hand_key_point_from_image.py
'''
import cv2
...
def core(image, is_video=False):
    ...
    if (results.multi_hand_landmarks != None):
        ...
        if (right_or_left == 'left'):
            ...
            vector.append(1)
        for i in range(21):
            vector.append(object_hand.landmark[i].x)
            vector.append(object_hand.landmark[i].y)
            vector.append(object_hand.landmark[i].z)
        hands.close()
        return vector
    else:
        hands.close()
...
if __name__ == '__main__':
    path = "model/20240527240527001001/dataset/1/1_7.jpeg"
